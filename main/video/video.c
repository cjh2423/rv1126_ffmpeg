...
static int venc_init(const VideoConfig *cfg) {
    ...
    if (RK_MPI_VENC_CreateChn(cfg->vi_chn_id, &venc_attr) != RK_SUCCESS) {
    ...
    if (RK_MPI_VENC_StartRecvFrame(cfg->vi_chn_id, &recv_param) != RK_SUCCESS) {
        LOG_ERROR("RK_MPI_VENC_StartRecvFrame failed\n");
        return -1;
    }

    g_venc_chn.enModId = RK_ID_VENC;
    g_venc_chn.s32DevId = 0;
    g_venc_chn.s32ChnId = cfg->vi_chn_id;
...
int rk_video_init(void) {
    ...
    g_video_run = 1;
    if (pthread_create(&g_venc_thread, NULL, venc_get_stream_thread, NULL) != 0) {
